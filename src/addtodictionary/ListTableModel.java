...
import static addtodictionary.RowTableModel.formatColumnName;
...
//
    ...
    public static ListTableModel createModelFromHQL(List<Entries> list, ArrayList<String> columnNames)
            throws SQLException {
        ListTableModel model = new ListTableModel(columnNames);
        model.setModelEditable(false);
       ...
       /* for (int i = 1; i <= 3; i++) {
            try {
                String className = columnNames.get(i);
                model.setColumnClass(i - 1, Class.forName(className));
            } catch (Exception exception) {
                System.out.println("Error setting classname");
            }
        }*/
        //  Get row data
        ...
        for (int i = 0; i < list.size(); i++) {
            ArrayList<Object> row = new ArrayList<Object>(3);
                Integer n = list.get(i).getN();
                String word = list.get(i).getWord();
                String def = list.get(i).getDefinition();
                row.add(n);
                row.add(word);
                row.add(def);
            data.add(row);
